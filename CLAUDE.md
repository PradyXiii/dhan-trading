...
| Lazy-imported lib not installed → feature shows 0.000 importance | All HMM regime features showed 0.000 importance even though code looked correct | The `try: import hmmlearn` block silently fell to `_HMM_OK=False` because the lib was never installed → fallback returned constant 0.33 → constant column = zero variance = zero importance. Always run `pip install --break-system-packages <lib>` on the VM BEFORE running `--analyze`. PEP 668 blocks plain pip on Debian-managed systems; use the flag. |
